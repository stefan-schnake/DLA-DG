function [C,S,D] = DLA4_HB_PROJ_FE(x,v,k,C,S,D,dt,Awave,BC)
...
%%%SSP-RK3
F = @(g) g - dt*applyAonS(g,C1,D1,C,D,S,Awave);
if BC.use
    S1 = F(S_);
    for l=1:NN
...
function CLuD = applyAonS(S,C,D,C0,D0,S0,Awave)
%%%Updates S via the DLA update

CLuD = zeros(size(S));
for l=1:size(Awave,1)
    CLuD = CLuD + (C'*Awave{l,1}*C0)*S0*(D0'*Awave{l,2}'*D);
end
