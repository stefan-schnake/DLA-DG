...
%dt = 0.5*(2/N^2);
%dt = .1;
%CFL = (max(abs(xx))+max(abs(vv)))/(2*k+1)*((x(2)-x(1)))^2;
%CFL = 1/N^2;
%dt = 0.8*CFL;
dt = 1e-5;
%CFL = (5/4)*(1/73536);
...
G = buildSeparableVOnlySource(x,v,k,@(v) 0*v+(vv(2)-vv(1)),one);
p = 1;
...
%Get rank-r approximation
C = U(:,1:r);
D = V(:,1:r);
S = Sig(1:r,1:r);

%Add conservative components
[C,S,D] = conservativeIC(x,v,k,C,S,D,G,G);
...
%updateDLA = @(C,S,D) DLA2_CONS_SSP(x,v,k,C,S,D,p,dt,A,BC);
%updateDLA = @(C,S,D) DLA2_CONS_BE(x,v,k,C,S,D,p,dt,A,BC);
updateDLA = @(C,S,D) DLA2_CONS_BE2(x,v,k,C,S,D,p,dt,A,BC);
%updateDLA = @(C,S,D) DLA2_CONS_HYB(x,v,k,C,S,D,p,dt,A,BC);
