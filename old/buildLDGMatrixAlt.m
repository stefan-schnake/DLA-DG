function [L] = buildLDGMatrixAlt(x,v,k)
%Stiffness matrix for Laplacian -Delta with homogeneous Neumann BCs.
% (\grad g , tau)_\W - <[g],{tau}>_{E_h^I} + <[g],C[tau]>_{E_h^I}
...
for i=1:num_x   
    ...
    if i > 1 %lower edge integral
        ...
        %Jump term
        jump1 = -leg_edge_vals(:,1)/sqrt(jac_x);
        jump2 = -leg_edge_vals(:,1)/sqrt(jac_x);
        block_x{3,i} = block_x{3,i} + jump2*jump1';
        ...
        %Jump term
        jump1 = -leg_edge_vals(:,1)/sqrt(jac_x);
        jump2 =  leg_edge_vals(:,2)/sqrt(jac_x);
        block_x{4,i} = block_x{4,i} + jump2*jump1';
    end
    ...
    if i < num_x %upper edge integral
        ...
        %Jump term
        jump1 = leg_edge_vals(:,2)/sqrt(jac_x);
        jump2 = leg_edge_vals(:,2)/sqrt(jac_x);
        block_x{3,i} = block_x{3,i} + jump2*jump1';
        ...
        jump1 =  leg_edge_vals(:,2)/sqrt(jac_x);
        jump2 = -leg_edge_vals(:,1)/sqrt(jac_x);
        block_x{5,i} = block_x{5,i} + jump2*jump1';
    end
...
for i=1:num_v    
    ...
    if i > 1 %lower edge integral
        ...
        jump1 = -leg_edge_vals(:,1)/sqrt(jac_x);
        jump2 = -leg_edge_vals(:,1)/sqrt(jac_x);
        block_v{3,i} = block_v{3,i} + jump2*jump1';
        ...
        jump1 = -leg_edge_vals(:,1)/sqrt(jac_x);
        jump2 =  leg_edge_vals(:,2)/sqrt(jac_x);
        block_v{4,i} = block_v{4,i} + jump2*jump1';
    end
    ...
    if i < num_v %upper edge integral
        ...
        jump1 =  leg_edge_vals(:,2)/sqrt(jac_x);
        jump2 = -leg_edge_vals(:,1)/sqrt(jac_x);
        block_v{5,i} = block_v{5,i} + jump2*jump1';
    end
