function [C,S,D] = DLA2_SSP(x,v,k,C,S,D,dt,A,BC)
%% DLA
...
%%%SSP-RK3
F = @(g) g - dt*applyAonS(x,v,k,g,C1,D1,A);
F1 = F(Svec) - dt*reshape(C1'*BCc{1}*D1,[],1);
F2 = (3/4)*Svec + (1/4)*(F(F1) - dt*reshape(C1'*BCc{3}*D1,[],1));
Svec = (1/3)*Svec + (2/3)*(F(F2) - dt*reshape(C1'*BCc{2}*D1,[],1));
