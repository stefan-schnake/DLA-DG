...
%evalScripts
F1 = @(g) evalBurgers2DPeriodic(x,v,k,g);

updateDLA = @(C,S,D) DLAfunc2(x,v,k,C,S,D,dt,F1);
...
%% SSP-RK3
F = @(g) g - dt*evalBurgers2DPeriodic(x,v,k,g);
f1 = F(uu);
...
figure(5)
plotVec(x,v,k,u,@(x,v) soln(x,v,t));
sgtitle("DLA Solution r = "+num2str(R(end)))
fprintf('-- Norm of Full: %e\n',norm(uu));
figure(6)
plotVec(x,v,k,uu,@(x,v) soln(x,v,t));
