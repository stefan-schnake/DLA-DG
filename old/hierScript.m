...
updateDLA = @(C,S,D) DLA3_HB_SSP(x,v,k,C,S,D,dt,Awave,BC-SC,FMWT);
...
elseif adapt2
    ...
    %[C,S,D] = updateDLA(C,S,D);
    %updatePDE = @(u) update_RK3_SSP(u,dt,A,BC-SC);
    updatePDE = @(u) update_FE(u,dt,A,BC-SC);
    [C,S,D] = AdaptiveDLAWithInitv3(x,v,k,C,S,D,updateDLA,updatePDE,adapt_tol,i,C0,S0,D0,FMWT);
elseif hier_adapt
    [C,S,D] = updateDLA(C,S,D);
    [C,S,D] = initHierAdapt(C,S,D,hier_tol);
else
...
%uu = expm(-A*(i*dt))*u0;
%%%SSP-RK3
u1 = uu - dt*A*uu - dt*BC(:,1) + dt*SC(:,1);
u2 = (3/4)*uu + (1/4)*(u1-dt*A*u1-dt*BC(:,3)+dt*SC(:,3));
uu = (1/3)*uu + (2/3)*(u2-dt*A*u2-dt*BC(:,2)+dt*SC(:,2));
...
myhist = [myhist [i;r;sing(end);norm(u-uu)]];
...
if plotbool
    ...
    %figure(7)
    %plotVec(x,v,k,uu_lr,@(x,y) soln(x,y,i*dt));
...
function u = update_FE(u,dt,A,SF)
    %Update by SSP-RK3
    u  = u - dt*A*u-dt*SF(:,1);
end
