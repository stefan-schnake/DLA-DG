function [C,S,D] = DLA2_HB_BE(x,v,k,C,S,D,dt,A,BC,FMWT)
%% DLA in hierarchical basis

%Convert BCs
%r = size(S,1);
BCc{1} = convertVectoMat(x,v,k,BC(:,3));
%[UB,SB,VB] = svd(BCc{1});
...
Kvec = gmres(@(g) g + dt*applyAonK(x,v,k,g,D,A,FMWT),Kvec - dt*(reshape(FMWT*(BCc{1}*(FMWT'*D)),[],1)),numel(Kvec),1e-10,numel(Kvec));
...
Lvec = gmres(@(g) g + dt*applyAonL(x,v,k,g,C,A,FMWT),Lvec - dt*(reshape(FMWT*(BCc{1}'*(FMWT'*C)),[],1)),numel(Lvec),1e-10,numel(Lvec));
...
Svec = gmres(@(g) g + dt*applyAonS(x,v,k,g,C1,D1,A,FMWT), Svec - dt*(reshape((FMWT'*C1)'*BCc{1}*(FMWT'*D1),[],1)),numel(Svec),1e-10,numel(Svec));
