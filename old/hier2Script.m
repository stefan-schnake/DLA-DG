...
if test == 1
    ...
    BC.use = 1;
    RHS = @(t,dt) ...
    [buildDirichletBC(x,v,k,@(x,y) soln(x,y,t-dt)), ...
     buildDirichletBC(x,v,k,@(x,y) soln(x,y,t-0.5*dt)), ...
     buildDirichletBC(x,v,k,@(x,y) soln(x,y,t))] - ...
    [buildNonSeparableSource(x,v,k,@(x,y) source(x,y,t-dt)),...
     buildNonSeparableSource(x,v,k,@(x,y) source(x,y,t-0.5*dt)),...
     buildNonSeparableSource(x,v,k,@(x,y) source(x,y,t))];
    BC.vec = RHS(t,dt);
end
...
elseif adapt3
    ...
    %[C,S,D] = AdaptiveDLAResdiual_SSP_RK2(x,v,k,C,S,D,dt,adapt_tol,Awave,BC,FMWT);
    %[C,S,D] = AdaptiveDLAResdiual_FE(x,v,k,C,S,D,dt,adapt_tol,Awave,BC,FMWT);
...
%if BC.use
%    u1 = uu - dt*A*uu - dt*BC.vec(:,1);
%    uu = (1/2)*uu + (1/2)*(u1-dt*A*u1-dt*BC.vec(:,3));
%else
%    u1 = uu - dt*A*uu;
%    uu = (1/2)*uu + (1/2)*(u1-dt*A*u1);
%end
%uu = u1;

% if BC.use
%     U1 = UU - dt*applyMatA(UU,Awave) - dt*BC.vec(:,1);
...
% 
% if BC.use
%    LTEU1 = U - dt*applyMatA(U,Awave) - dt*BC.vec(:,1);
...
%myhist [i;rank;smallest singular value;residual;LTE;actual error];
if fullgrid
...
if plotbool && ( mod(i,30) == 0 || i == 1 || lastplot)
    u = convertMattoVec(x,v,k,FMWT'*C*S*D'*FMWT);
    %u_sol = buildNonSeparableSource(x,v,k,@(x,y) soln(x,y,i*dt));
    figure(5)
    plotVec(x,v,k,u,@(x,y) soln(x,y,t));
    sgtitle('DLA Solution')
    %figure(6)
    %plotVec(x,v,k,uu,@(x,y) soln(x,y,t));
    ...
    semilogy(myhist(6,:),myhist(4,:))
    yyaxis right
    plot(myhist(6,:),myhist(2,:))
    yyaxis left
    legend({'u-uu','LTE','rank'},'location','best')
...
if (hier_adapt+adapt+adapt2+adapt3+adapt4) && plotbool
    ...
    figure(8)
    title("Metrics - Test = " + num2str(test) + "; tol = " + num2str(adapt_tol));
...
if savebool
    ...
    cd(fold)
    %figure(5)
    %saveas(gcf,"test="+num2str(test)+"-tol="+num2str(adapt_tol)+"_DLA.eps",'epsc');
    %figure(6)
