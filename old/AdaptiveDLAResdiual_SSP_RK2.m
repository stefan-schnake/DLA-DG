function [C,S,D] = AdaptiveDLAResdiual_SSP_RK2(x,v,k,C,S,D,dt,tol,Awave,BC,FMWT)
...
%Update
[C,S,D] = DLA3_HB_SSP_RK2(x,v,k,C,S,D,dt,Awave,BC,FMWT);
%Compute SVD of S
[S_x,Sig,S_v] = svd(S);
Sig = diag(Sig);
...
elseif r < size(C,1) %Add most relavant vectors from residual
    ...
    if BC.use
        FF = FMWT*convertVectoMat(x,v,k,0.5*(BC.vec(:,1)+BC.vec(:,3)))*FMWT';
    else
        FF = zeros(size(C,1)); 
    end
    %[C1,S1,D1] = svds(@(x,trans) firstOrderResidual(C0,S0,D0,Awave,FF,x,trans,Chalf,Dhalf),[1,1]*size(C,1),15);
    [C1,S1,D1] = svd(firstOrderResidual(C0,S0,D0,Awave,FF,eye(size(Chalf,1)),'notransp',Chalf,Dhalf));
    ...
    %[C2,R1.mat] = qr([C0 Chalf C1],0);
    %[D2,R2.mat] = qr([D0 Dhalf D1],0);
    %Remove duplicates
    %[R1.U,R1.S,R1.V] = svd(R1.mat);
    %R1.rank = sum(diag(R1.S) > 1e-14);
    %[R2.U,R2.S,R2.V] = svd(R2.mat);
    %R2.rank = sum(diag(R2.S) > 1e-14);
    %rr = max([R1.rank R2.rank]);
    %C2 = C2*R1.U(:,1:rr);
    %D2 = D2*R2.U(:,1:rr);
    %resid1.mat = firstOrderResidual(C0,S0,D0,Awave,FF,eye(size(C2,1)),'notransp',C2,D2);
    %[resid1.U,resid1.S,resid1.V] = svd(resid1.mat);
    ...
    S2 = DLA3_HB_SSP_RK2_S_Only(x,v,k,C2,S2,D2,dt,Awave,BC,FMWT);
    C = C2; D = D2; S = S2;
    [S_x,Sig,S_v] = svd(S2);
    Sig = diag(Sig);
    fprintf('-- Adaptive: Smallest singular value is %e with tolerance %e\n',Sig(end),tol);
...
function Y = firstOrderResidual(C,S,D,Acell,F,X,trans,C1,D1)
