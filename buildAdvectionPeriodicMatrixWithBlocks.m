function [Acell] = buildAdvectionPeriodicMatrixWithBlocks(x,v,k)
...
for i=1:num_x   
    ...
    block_x{4,i} = 1/2*(leg_vals/sqrt(jac_x).*abs(quad_x))*(w_ref.*leg_vals'/sqrt(jac_x))*jac_x;
...
for i=1:num_v    
    ...
    block_v{4,i} = 1/2*(leg_vals/sqrt(jac_v).*abs(quad_v))*(w_ref.*leg_vals'/sqrt(jac_v))*jac_v;
...
for i=1:num_x
    ...
    %Grad
    grad = -block_x{2,i} + block_x{5,i} - 0.5*block_x{6,i};
    ...
    grad = block_x{7,i} - 0.5*block_x{8,i};
    J(count:count+(k+1)^2-1) = repmat(indices,(k+1),1);
    ...
    grad = block_x{9,i} - 0.5*block_x{10,i};
    J(count:count+(k+1)^2-1) = repmat(indices,(k+1),1);
...
for j=1:num_v
    ...
    %Grad
    grad = -block_v{2,j} + block_v{5,j} + 0.5*block_v{6,j};
    J(count:count+(k+1)^2-1) = repmat(indices,(k+1),1);
    I(count:count+(k+1)^2-1) = reshape(repmat(indices,1,(k+1)),[],1);
    S(count:count+(k+1)^2-1) = grad(:);
    count = count + (k+1)^2;
    ...
    grad = block_v{7,j} + 0.5*block_v{8,j};
    J(count:count+(k+1)^2-1) = repmat(indices,(k+1),1);
    I(count:count+(k+1)^2-1) = reshape(repmat(indices_dn,1,(k+1)),[],1);
    S(count:count+(k+1)^2-1) = grad(:);
    count = count + (k+1)^2;
    if j<num_x
        ...
        indices_up = indices + (k+1) - num_v*(k+1);
    end
    grad = block_v{9,j} + 0.5*block_v{10,j};
    J(count:count+(k+1)^2-1) = repmat(indices,(k+1),1);
    I(count:count+(k+1)^2-1) = reshape(repmat(indices_up,1,(k+1)),[],1);
    S(count:count+(k+1)^2-1) = grad(:);
    count = count + (k+1)^2;
