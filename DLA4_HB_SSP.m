function [C,S,D] = DLA4_HB_SSP(x,v,k,C,S,D,dt,Awave,BC,FMWT,tol)
...
%Uses thresholding in Lubich 2021 paper. 
...
K_ = (1/3)*K + (2/3)*(F(F2)- dt*FMWT*(BCc{2}*(FMWT'*D)));
...
L_ = (1/3)*L + (2/3)*(F(F2) - dt*FMWT*(BCc{2}'*(FMWT'*C)));
...
r = numel(sigma);
for rr = 1:numel(sigma)
    if norm(sigma(rr+1:end)) <= tol*norm(sigma)
        r = rr;
        break
    end
end
